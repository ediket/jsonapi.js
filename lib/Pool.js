...
class Pool {
  ...
  push (idx) {
    ...
    let deleteRequest = _.map(removed, rid => {
      ...
      if (id) {
        return this.sync.delete(this.getRemote(type, id))
          .then(response => delete this.pool[rid],
            response => {
              this._saveData(beforeCommit[rid], rid);
    ...
    let postOrPatchRequest = _.map(changedOrAdded, rid => {
      ...
      if (!this.pool[rid]) { return; }
      // not change
      if (_.isEqual(afterCommit[rid], beforeCommit[rid])) { return; }

      let type = this.pool[rid].get('type');
      let id = this.pool[rid].get('id');
      let delta = diff(beforeCommit[rid], afterCommit[rid]);

      if (id) {
        _.extend(delta, { type, id });
        return this.sync.patch(this.getRemote(type, id),
                               this._toRequest(delta))
      ...
      if (op === 'add') {
        return this.sync.post(this.getRemote(type, id, relation),
          ...
          .then(response => {
            ...
            response => this._saveResponse(response);
          });
      }
  ...
  _saveResponse (response, rid) {
    ...
    let mainResource = this._saveData(response.data, rid);
    let includedResources = _.map(
      response.included, data => this._saveData(data));
    return mainResource;
  ...
  _remove (rid) {
    ...
    if (_.isArray(rid)) {
      return _.map(rid, data => this._remove(rid));
    }
