...
class RestPool extends Pool {
  ...
  patch (resource, attributes, options) {
    ...
    let setArguments = _.toArray(arguments).slice(1);
  ...
  get (urlOrLinkage, options) {

    let url = this._toURL(urlOrLinkage);
  ...
  _toURL (urlOrLinkage) {
    ...
    let url;
    if (_.startsWith(urlOrLinkage, '/')) {
      url = urlOrLinkage;
    }
    else {
      let linkage = urlOrLinkage;
  ...
  _refreshOrCreate (data) {
    ...
    let resource = this.pool[data.links.self];
